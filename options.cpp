...
const int MAX = 30; // max na pwede iadd sa both database
...
void button(int x, int y, int l, const string& a, int p)//unang parameter x coord, syempre sunod yung y coord, yung l kung gaanon kalaki yung button, last is yung pangalan
{
	...
	gotoxy(x, y + 1); cout << char(186); gotoxy(x + l + 1, y + 1); cout << char(186);// two hori 
...
void sorting(int a) { //SOTING FUNCTIONN (goods)
	...
	if (a == 1) {
		for (int a = 0; a < n - 1; ++a) {
			for (int i = 0; i < n - 1; ++i) {
				if (tempst[i].title > tempst[i + 1].title) {
					temp = tempst[i];
					tempst[i] = tempst[i + 1];
					tempst[i + 1] = temp;
		...
		for (int a = 0; a < n - 1; ++a) {
			for (int i = 0; i < n - 1; ++i) {
				if (tempst[i].author > tempst[i + 1].author) {
					temp = tempst[i];
					tempst[i] = tempst[i + 1];
					tempst[i + 1] = temp;
				}
		...
		for (int a = 0; a < n - 1; ++a) {
			for (int i = 0; i < n - 1; ++i) {
				if (tempst[i].location > tempst[i + 1].location) {
					temp = tempst[i];
					tempst[i] = tempst[i + 1];
					tempst[i + 1] = temp;
				}
	...
	for (const auto& book : tempst) {
		...
		gotoxy(67, 12 + z); cout << book.availability;
		gotoxy(52, 12 + z); cout << book.year;
...
m:
	...
	c(7); button(21, 5, 15, "Author", 0);
	c(7); button(39, 5, 15, " Category", 0);
	c(7); button(57, 5, 14, "Back", 0);
	...
	if (counter == 1) {
		...
		c(4); button(39, 5, 15, "", 0);
	}
	else if (counter == 4) {
		c(4); button(57, 5, 14, "", 0);
	}
...
void delRent(string& s) { //delete rent
	system("cls");
	table(5, 1, 1, 1);
...
void returnb(string t, string& search) { // return book function
	vector<Rent> tempstor;
	string line, bname, btitle, bauthor, duration ,tempt;
	int dur, time;
	int x = 40, y = 20;
	dur = stoi(t);
	...
	if (!Record.is_open()) {
		gotoxy(45, 2); cout << "Unable to open Renting_Record.txt!" << endl;
		return;
	}
	while (getline(Record, line)) {
		stringstream ss(line);
	...
	hc(0);
	gotoxy(x, y - 1); cout << char(218) << string(39, char(196)) << char(191);
	gotoxy(x, y); cout << char(179);
	gotoxy(x + 1, y); cout << "How long do you borrowed the book:" << char(179);
	gotoxy(x + 40, y); cout << char(179);
	gotoxy(x, y + 1); cout << char(192) << string(39, char(196)) << char(217);
	gotoxy(x + 37, y); cin >> time;

	fstream bRecord("Book_Record.txt", ios::in | ios::out);
	vector<string> lines;  // Vector to store updated lines

	if (!bRecord.is_open()) {
		gotoxy(45, 2); cout << "Unable to open Book_Record.txt!" << endl;
		return;
	}
	...
	if (bRecord.is_open()) {
		...
		gotoxy(45, 2); cout << "Unable to open Book_Record.txt for writing!" << endl;
		return;
	}
	...
	f:
		for (int q = 0; q < 5; q++) {
			gotoxy(40, 18 + q); cout << string(100, ' ');
		}
		for (int k = 0; k < 3; k++) {
			gotoxy(35, (y + 1) + k); cout << char(179);
			gotoxy(85, (y + 1) + k); cout << char(179);
		}
		gotoxy(35, y); cout << char(218) << string(49, char(196)) << char(191);
		gotoxy(x, y + 1); cout << "You exceeded the time for borrowing the book!";
		gotoxy(x, y + 2); cout << "We will charge you 300php for overdue fee";
		gotoxy(35, y + 4); cout << char(192) << string(49, char(196)) << char(217);
		gotoxy(54, y + 3); cout << "[          ]";
		gotoxy(58, y + 3); cin >> fee;
		if (fee > 300) {
			gotoxy(52, 19); c(4); cout << "Exceeded amount!"; c(7);
			Sleep(1000);
			goto f;
		}
		else {
			for (auto& rent : tempstor) {
				if (rent.bname == search) {
					rent.duration = "returned";  // Mark as returned
				}
			}
			ofstream record("Renting_Record.txt");
			if (!record.is_open()) {
				gotoxy(45, 2); cout << "Unable to open Renting_Record.txt for writing!" << endl;
				return;
			}
			...
			gotoxy(55, 18); c(3); cout << "Thank you!"; c(7);
			Sleep(1000);
			return;
		}
	}
...
M:
			...
			if (d == -32) {
				...
				if (records.is_open()) {
					bool found = false;
						...
						gotoxy(12, 14); cout << string(18, ' ');
						gotoxy(40, 14); cout << string(20, ' ');
						gotoxy(71, 14); cout << string(20, ' '); //pang erase sa extra lines
						gotoxy(101, 14); cout << string(8, ' ');
	...
	case 1: // return
		...
		case 13:
			if (tempdur == "returned") {
				gotoxy(5, 2); cout << "This rented book is already returned!";
				Sleep(1000);
				goto a;
			}
			else if (search != "") {
